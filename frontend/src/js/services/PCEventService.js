...
angular.module('PCloudSyncServer')
    .service('PCEvents', ['$rootScope', '$http', '$timeout', 'syncServerConfig', function ($rootScope, $http, $timeout, syncServerConfig) {
        'use strict';
        ...
        function successFn (data) {
            ...
            if (lastID > 0) {   // not emit events from first response
                data.forEach(function (event) {
                    ...
                    $rootScope.$emit(event.type, event);
                });
            }
            ...
            $timeout(function () {
                $http.get(syncServerConfig.syncUrl + '/events?since=' + lastID)
                    .success(successFn)
                    .error(errorFn);
            }, 500, false);
        }
        ...
        function errorFn (dummy) {
            $rootScope.$emit(self.OFFLINE);

            $timeout(function () {
                $http.get(syncServerConfig.syncUrl + '/events?limit=1')
                    .success(successFn)
                    .error(errorFn);
            }, 1000, false);
        }
